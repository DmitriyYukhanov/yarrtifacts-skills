...
Sign in at [yarrtifacts.com](https://yarrtifacts.com) → **API tokens** → Create token. Copy it
right away (it shows once) and set it where your agent runs:

```bash
export YARRTIFACTS_TOKEN=yarr_pat_…
```

The token can only upload or replace artifacts. Deleting, renaming, and domain settings stay in
the dashboard, so a leaked token can't do much harm — and you can revoke it there any time.
